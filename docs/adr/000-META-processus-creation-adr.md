...
stakeholders: ["@architecture-team", "@dev-team", "@vivo-marketplace"]
effort: "low"
---
...
Le projet **vivo-marketplace** vise à industrialiser et publier sur Microsoft Azure Marketplace une offre de type Virtual Machine (VM) intégrant la solution open source VIVO (Vitro + vivo-solr) sur Azure. Cette solution utilise un triplestore RDF comme base de données principale et est destinée aux universités, centres de recherche et organisations souhaitant déployer un knowledge graph institutionnel dans leur propre abonnement Azure.
...
Adopter un processus formalisé de création et gestion des ADR basé sur le modèle Michael Nygard, adapté pour vivo-marketplace.
...
100-ARCH-vivo-triplestore-architecture.md   # Architecture VIVO
200-INFRA-azure-vm-image-build.md           # Image VM Azure
300-SEC-tls-network-security.md             # Sécurité TLS et réseau
400-DATA-rdf-triplestore-choice.md          # Triplestore RDF
500-API-vivo-solr-integration.md            # Intégration Solr
...
| `ARCH` | 100-199 | Architecture | Design patterns, principes architecturaux VIVO |
| `INFRA` | 200-299 | Infrastructure | Azure VM, IaC, Packer, déploiement |
| `SEC` | 300-399 | Sécurité | TLS, réseau, RBAC, conformité Marketplace |
| `DATA` | 400-499 | Données | Triplestore RDF, Solr, ontologies, ETL |
| `API` | 500-599 | APIs | Intégrations VIVO, REST, Solr, monitoring |
...
- `architecture`: Décisions architecturales VIVO / triplestore / Solr
- `infrastructure`: Azure VM, Packer, Bicep, déploiement
- `security`: TLS, réseau, RBAC, conformité Microsoft Marketplace
- `data`: Triplestore RDF, Solr, ontologies VIVO
- `api`: Intégrations VIVO API, monitoring, REST
...
- Exemples pertinents : `java`, `azure`, `vm`, `docker`, `tomcat`, `solr`, `rdf`, `sparql`, `triplestore`, `packer`, `bicep`, `tls`, `marketplace`
...
- ✅ Décision architecturale significative (triplestore, image VM, Marketplace)
...
- [VIVO Open Source Project](https://vivoweb.org/)
...
|------|--------|------------|--------|
| 2026-02-21 | @architecture-team | Création initiale pour vivo-marketplace | Import et adaptation depuis og-nore |
