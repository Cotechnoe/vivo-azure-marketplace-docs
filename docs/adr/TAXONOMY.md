...
| Préfixe | Plage | Domaine | Exemples ADR vivo-marketplace |
|---------|-------|---------|-------------------------------|
| `META` | 000-099 | Méta-processus | ADR-000: Processus ADR |
| `ARCH` | 100-199 | Architecture | ADR-100: Architecture VIVO triplestore |
| `INFRA` | 200-299 | Infrastructure | ADR-200: Azure VM image build (Packer) |
| `SEC` | 300-399 | Sécurité | ADR-300: TLS + Network Security Groups |
| `DATA` | 400-499 | Données | ADR-400: Choix triplestore RDF |
| `API` | 500-599 | API/Intégrations | ADR-500: Intégration VIVO + Solr |
...
| Valeur | Description | Exemples contexte vivo-marketplace |
|--------|-------------|-------------------------------------|
| `architecture` | Décisions architecturales VIVO/triplestore | Patterns intégration Vitro + Solr |
...
| `data` | Triplestore RDF, Solr, ontologies | Choix TDB1 vs Fuseki, schéma Solr |
| `api` | Interfaces VIVO, REST, intégrations | API VIVO, monitoring endpoints |
...
**Aide décision (contexte vivo-marketplace)**:
- **Low**: Changement version mineure Tomcat, ajout monitoring endpoint
- **Medium**: Changement configuration Solr, ajustement sizing VM
- **High**: Changement triplestore RDF, base image OS VM
- **Critical**: Choix plateforme cloud, architecture VIVO core
...
| Valeur | Description | Métriques Typiques | Pertinence vivo-marketplace |
|--------|-------------|-------------------|------------------------------|
| `performance` | Latence, débit, scalabilité | p95 latency, RPS | Temps chargement VIVO, requêtes SPARQL |
| `security` | Auth, autorisation, encryption | CVE count, TLS grade | TLS 1.2+, NSG rules, Marketplace compliance |
| `reliability` | Disponibilité, tolérance pannes | Uptime %, MTBF | Disponibilité VM client, restart Tomcat |
...
| `usability` | Developer experience | Time to deploy | Déploiement simple pour université |
...
**Aide décision (contexte vivo-marketplace)**:
- **Easy**: Version Tomcat mineur, paramètre JVM
- **Moderate**: Version Solr, configuration réseau VM
- **Hard**: Triplestore RDF (migration données nécessaire)
...
**Aide décision (contexte vivo-marketplace)**:
- **Strategic**: Choix Azure Marketplace vs autre canal distribution, triplestore principal
- **Tactical**: Architecture image VM, pipeline CI/CD, certification Marketplace
- **Operational**: Script provisioning, version package, configuration JVM
...
#### Languages & Runtimes
- `java`, `bash`, `python`, `javascript`

#### Cloud & Infrastructure
- `azure`, `azure-marketplace`, `vm`, `bicep`, `terraform`, `packer`, `docker`

#### VIVO Stack
- `vivo`, `vitro`, `vivo-solr`, `tomcat`

#### Data & Semantic Web
- `rdf`, `sparql`, `triplestore`, `ontology`, `solr`, `tdb1`, `fuseki`, `jena`
...
### Recherche par tech_area VIVO
```bash
grep -l '"vivo"' docs/adr/*.md
```
...
### Exemples vivo-marketplace

```
000-META-processus-creation-adr.md          # META: 000-099
100-ARCH-vivo-triplestore-architecture.md   # ARCH: 100-199
200-INFRA-azure-vm-image-packer.md          # INFRA: 200-299
300-SEC-tls-network-security-groups.md      # SEC: 300-399
400-DATA-rdf-triplestore-choice.md          # DATA: 400-499
500-API-vivo-solr-integration.md            # API: 500-599
...
**Contexte spécifique vivo-marketplace**: La classification `compliance` est particulièrement importante dans ce projet en raison des exigences strictes de certification Microsoft Azure Marketplace et des obligations de licensing open source (VIVO est sous licence Apache 2.0).
